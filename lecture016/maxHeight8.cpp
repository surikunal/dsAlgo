...
int floodfill(vector<vector<bool>> &isdone, int sr, int sc, int er, int ec, string ans)
{
    ...
    {
        max_ = max(max_, floodfill(isdone, sr + 1, sc + 1, er, ec, ans + "D1 "));
    }
    if (isValidLocation(isdone, sr + 1, sc - 1))
    {
        max_ = max(max_, floodfill(isdone, sr + 1, sc - 1, er, ec, ans + "D2 "));
    }
    if (isValidLocation(isdone, sr - 1, sc + 1))
    {
        max_ = max(max_, floodfill(isdone, sr - 1, sc + 1, er, ec, ans + "D3 "));
    }
    if (isValidLocation(isdone, sr - 1, sc - 1))
    {
        max_ = max(max_, floodfill(isdone, sr - 1, sc - 1, er, ec, ans + "D4 "));
    }
    isdone[sr][sc] = false;
    return max_ + 1;
}
